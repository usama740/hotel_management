...
It provides endpoints to create, update, delete, retrieve, and list menu items.
...
class MenuUpdateView(APIView):
    ...
    @extend_schema(
        request=MenuSerializer,  # Define request body for PUT request as well
        responses={status.HTTP_200_OK: MenuSerializer}  # Success response format for update
    )
    ...
    @extend_schema(
        ...
        # Indicating successful deletion with no content in response body
    )
